...
def test_check_and_load_images(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    loaded_image_2, loaded_image_3 = engine._check_and_load_images(  # noqa: SLF001
        sequence_id='99', frame_id='111111'
    )
    ...
    try:
        ...
        loaded_image_2_new, loaded_image_3_new = engine._check_and_load_images(  # noqa: SLF001
            sequence_id='99', frame_id='222222'
        )
...
def create_mock_voxel_files(label_path: str, invalid_path: str, grid_n: int) -> None:
...
def test_return_process() -> None:
    """The process method must return the correct data. Test only non-tested parts."""
    data_dir = str(Path.cwd() / 'tmp')
    ...
    # Create mock files, but intentionally leave some file missing
    try:
        sequence_path = Path(data_dir) / 'sequences' / '99'
        calib_file = str(sequence_path / 'calib.txt')
        pose_file = str(sequence_path / 'poses.txt')
        ...
        if not image2_path.exists():
            image2_path.mkdir(parents=True)
        create_mock_lidar_file(str(lidar_pc_path / '111110.bin'))
        create_mock_image_files(str(image2_path / '111110.png'))
        create_mock_calib_file(calib_file)
        create_mock_pose_file(pose_file)
        vox_label_path = sequence_path / 'voxels' / '111110.label'
        vox_invalid_path = sequence_path / 'voxels' / '111110.invalid'
        if not Path(vox_label_path).exists():
            Path(vox_label_path).parent.mkdir(parents=True)
        create_mock_voxel_files(
            str(vox_label_path), str(vox_invalid_path), int(np.prod(engine.scene_dim))
        )

        # No static data processed yet
        with pytest.raises(AttributeError):
            _ = engine.static_data['sequence_id'] == '99'

        # Process the data and check the return value
        ret = engine.process(99, 111110)
        assert ret['sequence_id'] == '99'
        assert ret['frame_id'] == '111110'
        assert ret['timestamp'] == 11111.0
        ...
        assert np.allclose(ret['lidar_pc_labels'], np.zeros((1,), dtype=np.uint32))  # type: ignore[arg-type]
        assert np.allclose(ret['lidar_pc_label_colors'], np.zeros((4, 1), dtype=np.uint8))  # type: ignore[arg-type]
        assert np.allclose(ret['trajectory'], np.zeros((3, 1), dtype=np.float64))  # type: ignore[arg-type]
        assert ret['voxel'].shape == tuple(config['semantic_kitti']['grid_dims'])  # type: ignore[union-attr]
        assert np.all(np.isin(ret['voxel'], config['semantic_kitti']['voxel_class_ids']))  # type: ignore[arg-type]
        assert ret['fov_mask'].dtype == np.bool_  # type: ignore[union-attr]
        assert ret['fov_mask'].shape[0] == int(np.prod(engine.scene_dim))  # type: ignore[union-attr]
        assert ret['t_velo_2_cam'].dtype == np.float64  # type: ignore[union-attr]
        assert np.allclose(ret['t_velo_2_cam'], np.eye(4))  # type: ignore[arg-type]
    finally:
        shutil.rmtree(data_dir)
