...
class BackendEngine:
    ...
    def __init__(
        ...
        lidar_pc_loader: DataLoader,
        trajectory_loader: DataLoader,
        voxel_loader: DataLoader,
        **kwargs: dict # In case using config file to build the object
    ):
        # TODO: Import and use the real loaders
        ...
        # TODO: Declare all meta data attributes
        self.frequency = 10 # Hz

    def process(self, frame_id: int | str) -> dict:
        """Call the loading methods of the loaders and pack them into a dict to be passed to communication.
        
        Args:
            frame_id: The id of the frame to be processed.
        Returns:
            data: data dict to be passed to communication with keys <sensor_name> and value <sensor_data>
        ...
        self.files = [
            f
            for f in os.path.join(self.data_dir, self.sequence_id, str(frame_id))
            if (curr_frame_id := int(os.path.basename(f))) >= start_frame_id
        ]
    ...
    def collate_fn(
        ...
        """Collate the data into a single list of dicts instead of dict of lists. Each index represents a frame.
        ...
        for k, v in data.items():
            assert len(v) == len(data["frame_id"]), f"The length of {k} is not equal to the length of frame_id"
        return [
            {
    ...
    def spin(self):
        """Spin the engine until the end of the data, or get interrupted, or new signal is received."""
        pass
